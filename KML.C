...
static CONST KmlToken pLexToken[] =
{
	...
	{TOK_DISTANCE,   000001, 8,_T("Distance")},
...
static LPCTSTR szAnnunBitmap = NULL;
...
static VOID InitGlobal(KmlBlock* pBlock)
{
	...
	while (pLine)
	{
		...
		case TOK_MODEL:
			...
			{
				// Lewis
				if (isModelLewis(cCurrentRomType))
				{
					nCurrentHardware = HDW_LEWIS;
					break;
				}

				// Sacajawea
				if (isModelSaca(cCurrentRomType))
				{
					nCurrentHardware = HDW_SACA;
					break;
				}
...
static KmlLine* InitDigit(KmlBlock* pBlock)
{
	KmlLine* pLine = pBlock->pFirstLine;
	while (pLine)
	{
		...
		{
		case TOK_OFFSET:
			nLcdXOff = (UINT) pLine->nParam[0];
			nLcdYOff = (UINT) pLine->nParam[1];
			break;
		case TOK_SIZE:
			nLcdXPixel = (UINT) pLine->nParam[0];
			nLcdYPixel = (UINT) pLine->nParam[1];
			break;
		case TOK_DISTANCE:
			nLcdDistance = (UINT) pLine->nParam[0];
			break;
		case TOK_BITMAP:
			szAnnunBitmap = (LPCTSTR) pLine->nParam[0];
			break;
		case TOK_END:
			return pLine;
		default:
...
VOID ReloadButtons(WORD *Keyboard_Row, UINT nSize)
{
	UINT i;
...
VOID DrawAnnunciator(UINT nId, BOOL bOn, DWORD dwColor)
{
	...
	EnterCriticalSection(&csGDILock);		// solving NT GDI problems
	{
		if (bOn)
		{
			...
			dwColor = ((dwColor & 0xFF0000) >> 16) | ((dwColor & 0xFF) << 16) | (dwColor & 0xFF00);

			// create monochrome bitmap of annunciator
			hBmpDC  = CreateCompatibleDC(hMainDC);
			hBmp    = CreateBitmap(pAnnunciator[nId].nCx,pAnnunciator[nId].nCy,1,1,NULL);
			hBmp    = (HBITMAP) SelectObject(hBmpDC, hBmp);

			// get background color from upper left corner of annunciator
			cColor = SetBkColor(hMainDC,GetPixel(hMainDC,pAnnunciator[nId].nDx,pAnnunciator[nId].nDy));
			BitBlt(hBmpDC,
				0, 0,
				...
				SRCCOPY);
			SetBkColor(hMainDC, cColor);

			// create brush
			hBrush = CreateSolidBrush(dwColor);
			hBrush = (HBRUSH) SelectObject(hWindowDC, hBrush);
			...
			// destroy monochrome bitmap of annunciator
			hBmp = (HBITMAP) SelectObject(hBmpDC, hBmp);
			DeleteObject(hBmp);
			DeleteDC(hBmpDC);
		}
...
BOOL InitKML(LPCTSTR szFilename, BOOL bNoLog)
{
	...
	{
		if (!CreateAnnunBitmap(szAnnunBitmap)) // load it
		{
			PrintfToLog(_T("Cannot load Annunciator Bitmap %s."),szAnnunBitmap);
			goto quit;
		}
		PrintfToLog(_T("Annunciator Bitmap %s Loaded."),szAnnunBitmap);
	}

	CreateLcdBitmap();
	InitIO();								// init IO hardware
...
quit:
	if (bOk)
	{
		// HP28S, HP32SII and HP42S have object loading -> support dropped files
		DragAcceptFiles(hWnd,   cCurrentRomType == 'D'
							 || cCurrentRomType == 'N'
							 || cCurrentRomType == 'O');
