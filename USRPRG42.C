...
static INT DecodeUCByte(INT nPrevType, BYTE byCurrByte)
{
	...
	case SINGLE:
		// 2 byte
		if (   (byCurrByte >= 0x90 && byCurrByte <= 0xBF)
...
//
static BOOL Catalog(BOOL *pbFirst, CatLabel *pLbl)
{
	...
	while (nLoopStatus != DONE)
	{
		...
		{
			nByteType = DecodeUCByte(nByteType,Read2(dwAddrIndex));
			switch (nByteType)
			{
			case GLOBAL1:
			case GLOBAL2:
			...
			case TEXT:
				...
				if (nTextCount == 1)		// got label name
				{
					...
					DWORD dwPrevObjAddr = PrevGlobal(dwCurrAddr);
					if (dwPrevObjAddr != 0	// not end of chain
						&& (   DecodeUCByte(FIRST,  Read2(dwPrevObjAddr+0)) != GLOBAL1
							|| DecodeUCByte(GLOBAL1,Read2(dwPrevObjAddr+2)) != GLOBAL2
							|| DecodeUCByte(GLOBAL2,Read2(dwPrevObjAddr+4)) != GLOBAL_END
						   )
					   )
					{
					...
					// current program begins directly after GLOBAL_END or at end of chain
					pLbl->dwStartAddr = (dwPrevObjAddr != 0) ? (dwPrevObjAddr + 6) : (Read5(D_CONTEXT) + 10);
...
static BOOL PutUserCode(LPCTSTR szUCFile,HWND hWnd)
{
	...
	for (i = nItems; --i >= 0;)				// scan all elements in reversed order
	{
		...
		{
			CatLabel *pLbl = (CatLabel*) SendMessage(hWnd,LB_GETITEMDATA,i,0);
...
static BOOL CALLBACK SelectProgram(HWND hDlg, UINT message, DWORD wParam, LONG lParam)
{
	...
	case WM_INITDIALOG:
		...
		while (Catalog(&bFirst, &Lbl))
		{
			...
			CatLabel *pLbl = (CatLabel*) malloc(sizeof(*pLbl));
			if (pLbl == NULL) break;
