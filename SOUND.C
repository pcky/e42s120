...
static BYTE WaveLevel(WORD wOut)
{
	...
	switch (wOut)							// decode volume level from new state
	{
	case 0 << 10:
	case 3 << 10: // beeper pins have same level
		byWaveLevel = 1;
		break;
	case 1 << 10: // full negative
		byWaveLevel = 0;
		break;
	case 2 << 10: // full positive
		byWaveLevel = 2;
		break;
	default:
		_ASSERT(FALSE);
...
//
VOID SoundOut(CHIPSET_M* w, WORD wOut)
{
	static DWORD dwLastCyc;					// last timer value at beeper bit change

	enum MMUMAP eMap;
	DWORD dwCycles,dwDiffCycles,dwCpuFreq,dwSamples;
	BYTE  byWaveLevel;
	...
	dwDiffCycles = dwCycles - dwLastCyc;	// time difference from syncpoint in original Saturn cycles
	dwLastCyc = dwCycles;					// new syncpoint

	// theoretical CPU frequency from given T2CYCLES
	dwCpuFreq = T2CYCLES * 16384;

	if (dwDiffCycles > dwCpuFreq / 2 / 2)	// frequency < 2 Hz (because of CPU SLEEP)
		return;

	eMap = MapData(w->pc);					// get active memory controller

	// code executed by MEMC access
	if (eMap == M_NCE2 || eMap == M_NCE3 || eMap == M_NCE2S || eMap == M_NCE2S)
	{
		DWORD dwDiffSatCycles = dwDiffCycles;

		// estimated CPU cycles for MEMC access
		dwDiffCycles = (dwDiffSatCycles * 11) / 9;

		// adjust original CPU cycles and syncpoint
		w->cycles += (dwDiffCycles - dwDiffSatCycles);
		dwLastCyc += (dwDiffCycles - dwDiffSatCycles);
	}
	...
	byWaveLevel = 0x80 + (BYTE) (dwWaveVol * (WaveLevel(wOut) - WaveLevel(w->out)) / 4);
