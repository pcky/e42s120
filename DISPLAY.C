...
HDC  hLcdDC = NULL;
HDC  hAnnunDC = NULL;						// annunciator DC for Sacajawea
HDC  hMainDC = NULL;
...
// Sacajawea
static HDC     hSelAnnunDC;					// current annunciator DC
...
BOOL CreateMainBitmap(LPCTSTR szFilename)
{
	...
	hSelAnnunDC = hMainDC;					// use background bitmap for annunciators
	return TRUE;
}
...
VOID DestroyMainBitmap(VOID)
{
	if (hMainDC != NULL)
	{
		...
		hSelAnnunDC = NULL;					// reset annunciator DC
	}
	return;
}

//
// load Sacajawea annunciator bitmap
//
BOOL CreateAnnunBitmap(LPCTSTR szFilename)
{
	...
	hSelAnnunDC = hAnnunDC;					// use annunciator bitmap for annunciators
	return TRUE;
}

//
// destroy Sacajawea annunciator bitmap
//
