...
static INT_PTR CALLBACK Debugger(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		SendMessage(hDlg,WM_SETICON,ICON_BIG,(LPARAM) LoadIcon(hApp,MAKEINTRESOURCE(IDI_EMU42)));
		...
		if (!Chipset.bSlave)				// Pioneer series
		{
			EnableMenuItem(GetMenu(hDlg),ID_BREAKPOINTS_DOCODE,MF_GRAYED);
			bDbgCode = FALSE;
		}
	...
	case WM_COMMAND:
		...
		case ID_DEBUG_MEM_NCE1_S:
		case ID_DEBUG_MEM_NCE2_S:
		case ID_DEBUG_MEM_NCE3_S:         return OnMemMapping(hDlg,LOWORD(wParam));
...
static VOID UpdateProfileWnd(HWND hDlg)
{
	#define SCPU_FREQ 640000					// Sacajawea CPU frequency
	#define LCPU_FREQ 524288					// Lewis base CPU frequency
	...
	{
		dwFreq = SCPU_FREQ;
		dwEndFreq = (999 * 2 * SCPU_FREQ - SCPU_FREQ) / 2 / 1000;
	}
	else
	{
		dwFreq = ((DWORD) Chipset.IORam[RATECTL] + 1) * LCPU_FREQ / 4;
...
static INT_PTR CALLBACK RplObjView(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_INITDIALOG:
		dwRplPlatform = RPL_P2;				// Pioneer / Clamshell platform with RRP
...
static INT_PTR CALLBACK Settings(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		// fill model combo box and corresponding file edit box
		{
			...
			for (lpszModels = MODELS; *lpszModels != 0; ++lpszModels)
			{
